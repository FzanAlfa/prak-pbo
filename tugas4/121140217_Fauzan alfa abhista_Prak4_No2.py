...
class Antares(Robot):
    ...
    def lakukan_aksi(self, lawan):
        if self.jumlah_turn % 3 == 0 and self.jumlah_turn != 0: #ketika sudah menyerang dengan kelipatan 3 pasif akan keluar
            self.multiplier = 1.5
...
class Alphasetia(Robot):
    ...
    def lakukan_aksi(self, lawan):
        if self.jumlah_turn % 2 == 0 and self.jumlah_turn != 0: #setelah menyerang sebanyak kelipatan 2 pasfi akan keluar
            self.health += 4000
    ...
    def terima_aksi(self, damage):
        if self.health - damage <= 0:
            self.health = 0 #dilakukan pembatasan agar health point tidak minus
        else:
            self.health -= damage
            print(f"{self.name} menerima serangan sebanyak {damage} DMG")
...
class Lecalicus(Robot):
    ...
    def lakukan_aksi(self, lawan):
        if self.jumlah_turn % 4 == 0 and self.jumlah_turn != 0: #pasif menyala setelah kelipatan 4
            self.health += 7000
            self.multiplier = 2.0
...
robotmu = random.randint(1, 3) #menggunakan random 1-3 agar komputer bisa memilih robotnya sendiri
...
while robot_1.health > 0 and robot_2.health > 0:
    ...
    tangan_2 = random.randint(1, 3)  #menggunakan random 1-3 agar komputer dapat memilih guntinng batu kertas sendiri
    print(f"Pilih tangan robot lawan ({robot_1.name}]): {tangan_2}")

    if tangan_1 == tangan_2:
        print("Seri!")
    elif (tangan_1 == 3 and tangan_2 == 2) or (tangan_1 == 1 and tangan_2 == 3) or (tangan_1 == 2 and tangan_2 == 1):
...
print()
if robot_1.health <= 0:
    print(f"{robot_1.name} mati")
    print("kamu kalah")
else:
    print(f"{robot_2.name} mati")
    print("kamu menang")